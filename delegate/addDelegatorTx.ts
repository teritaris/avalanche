...
const networkID: number = 5 // info.getNetworkIDで取得
const avalanche: Avalanche = new Avalanche(ip, port, protocol, networkID)
const pchain: PlatformVMAPI = avalanche.PChain()
const bintools: BinTools = BinTools.getInstance()
const pKeychain: KeyChain = pchain.keyChain()
const privKey: string = "PrivateKey-2acWSDw3h7eXkJFzqZCNWWnaPYa1hwBGJbLvoDLtzNE6vUk9pD"
...
const nodeID: string = "NodeID-2pPHh62vcxcs2UTtujL1HdAVJAT1u1XNs"
const startTime: BN = UnixNow().add(new BN(60 * 1))
// const endTime: BN = startTime.add(new BN(2630000)) // 30日
const endTime: BN = startTime.add(new BN(60 * 60 * 24)) // 24h
